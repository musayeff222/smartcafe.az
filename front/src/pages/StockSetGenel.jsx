...
import UpdateStockSetForm from "../components/UpdateStockSetForm";
...
function Stok() {
  ...
  useEffect(() => {
    const fetchRawMaterials = async () => {
      console.log("detailsItem.id:", detailsItem?.id);
      if (detailsItem) {
        try {
  ...
  const handleClickDelete = async (item) => {
    const confirmed = window.confirm("Silməyə əminsiniz?");

    if (confirmed && item) {
      try {
        await axios.delete(`${base_url}/stock-sets/${item}`, getAuthHeaders());

        handleUpdateItem();
        toast.success("Set uğurla silindi!", {
          autoClose: 1000,
        });
      } catch (error) {
        toast.error("Set silinərkən xəta baş verdi!");
        console.error("Set silinərkən xəta baş verdi", error);
      }
    }
  };
  ...
  return (
    <>
      ...
      <section className="p-4">
        ...
        <div className="border border-t-0  py-3 px-3 flex flex-col md:flex-row gap-4">
          {!addStok ? (
            <>
              <div className="w-full md:w-3/4">
                <div className="flex flex-col md:flex-row items-center gap-3 mb-3">
                  <p className="my-2">Siyahida toplam {filteredItems.length} stok vardir.</p>
                  <button
                  ...
                  <table className="w-full text-left border rounded bg-[#fafbfc]">
                    <thead className="border-b border-gray-400 bg-gray-100">
                      <tr className="border-b border-gray-300">
                        ...
                        <th className="p-3 font-semibold">Yenilə</th>
                    ...
                    <tbody>
                      {filteredItems.map((item) => (
                        <tr
                          ...
                          <td className="text-red-900 cursor-pointer font-bold">
      ...
      {showDetails && detailsItem && (
        <>
          <div className="fixed inset-0 flex justify-center items-center bg-black/30 backdrop-blur-sm z-50 w-full">
            <UpdateStockSetForm
